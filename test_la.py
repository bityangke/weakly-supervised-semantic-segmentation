...
for s in segmentation_class:
    ...
    La_crf_p = np.load('./../../../psa/psa/VGG_LA_CRF/'+s.split('.')[0]+'.npy')
    #temp = Image.fromarray(La_crf_p[0]).resize((h, w), Image.LANCZOS)
    #La_crf = np.asarray(temp)    
    ...
    for key in La_crf_p:
        #print(key)
        #print(La_crf_p[key])
        #print( La_crf_p[key].shape )
        width, height = La_crf_p[key].shape
        H_image = scipy.misc.toimage(np.asarray(La_crf_p[key]), cmin = 0, cmax = 255, pal = colors_map, mode = 'P')#.show()
    ...
    #print(La_crf.keys())  
    sess = tf.Session()
    for i in La_crf.keys():
        gt = np.where((GT==(i)),1,0)
        #la_crf = np.where((La_crf[i]>=0.9),1,0)
        la_crf = La_crf[i]
        in_, un_ = sess.run([IN_,UN_], feed_dict={x:la_crf, y:gt}) 
