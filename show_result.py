...
for name in segmentation_class:
    if name in file_list:
        if count < 0:
            count = count + 1
            continue
        predict = Image.open("./predict_result/" + name + ".png")
        width, height = predict.size
        image = Image.open("../../../psa/psa/VOC2012/JPEGImages/" + name + ".jpg").resize((width,height))
        ground_truth = Image.open("../../../psa/psa/VOC2012/SegmentationClass/" + name + ".png").resize((width,height))
        ...
        if (count+1) % 5 == 0:
            ...
            plt.savefig("./show_result/" + str(count-4) + "_to_" + str(count+1) + ".png")
            #plt.show()
            plt.close()
            print(count)
            fig = plt.figure(figsize=(63,47)) 
